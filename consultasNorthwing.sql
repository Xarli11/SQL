...
SELECT product_id, product_name, (units_in_stock * unit_price) AS InvNum FROM products;
-- Cuanto vale el punto de reorden 
SELECT product_id, product_id, (units_in_stock * unit_price) AS InvNum FROM products;
-- Mostrar una consulta con Productid, productname y precio, el nombre del producto debe estar en mayuscula 
SELECT product_id, UPPER(product_name) AS NameProduct, unit_price FROM products;
...
--Obtener de la tabla de Productos las columnas productid, ProductName, UnitPrice, UnitsInStock cuyas existencias esten entre 50 y 
...
-- Obtener la información de la tabla de Products, Ordenarlos por Categoria de forma ascendente y por precio unitario de forma 
SELECT product_id, product_name, category_id, supplier_id FROM Products WHERE unit_price BETWEEN 25 AND 200 ORDER BY category_id ASC, unit_price DESC;
...
SELECT SUM(units_in_stock) as TotalExistencias FROM products;

--Promedio de los precios de la tabla de productos
SELECT AVG(unit_price) as PromedioPrecios FROM products;
...
SELECT MAX(unit_price) AS PrecioMaximo, MIN(unit_price) AS PrecioMinimo, AVG(unit_price) AS PrecioPromedio
FROM products;

--Agrupacion
--Numero de productos por categoria
SELECT category_id, COUNT(*) AS NumeroProductosCat FROM products GROUP BY category_id;

--Obtener el precio promedio por proveedor de la tabla de productos
SELECT supplier_id, AVG(unit_price) AS PrecioPromedio FROM products GROUP BY supplier_id;

--Obtener la suma de inventario (UnitsInStock) por SupplierID De la tabla de productos (Products)
SELECT supplier_id, AVG(unit_price) AS PrecioMedio FROM products GROUP BY supplier_id;

--Contar las ordenes por cliente de la tabla de orders
SELECT customer_id, COUNT(*) AS NumOrdenes FROM orders GROUP BY customer_id;

--Contar las ordenes por empleado de la tabla de ordenes unicamente del empleado 1,3,5,6
SELECT employee_id, COUNT(*) AS NumOrdenes FROM orders WHERE employee_id IN (1,3,5,6) GROUP BY employee_id;

--Obtener la suma del envío (freight) por cliente
SELECT customer_id, SUM(Freight) AS SumaEnvio FROM orders GROUP BY customer_id;
...
SELECT ship_city, SUM(Freight) AS SumaEnvio FROM orders WHERE ship_city IN ('Madrid', 'Sevilla', 'Barcelona', 'Lisboa', 'London') GROUP BY ship_city ORDER BY SumaEnvio;

--obtener el precio promedio de los productos por categoria sin contar con los productos descontinuados (Discontinued)
SELECT category_id, AVG(unit_price) AS PrecioPromedio FROM products WHERE discontinued = 0 GROUP BY category_id;
...
SELECT category_id, COUNT(*) AS NumProductos FROM products WHERE unit_price BETWEEN 10 AND 60 AND units_in_stock > 12 GROUP BY category_id;
...
SELECT category_id, SUM(units_in_stock) AS SumaExistencias FROM products WHERE supplier_id IN (17, 19, 16) GROUP BY category_id HAVING SUM(units_in_stock) < 100 ORDER BY SumaExistencias;
